...
class PIDRandomPositionSimulation:
    def __init__(self):
        ...
        self.dt = 0.9  # Reduced time step for finer control

        # Initialize PID controllers for each axis with updated gains
        self.pid_x = PID(2.0, 0.0, 0.05, sample_time=self.dt)
        self.pid_y = PID(2.0, 0.0, 0.05, sample_time=self.dt)
        self.pid_z = PID(2.0, 0.0, 0.05, sample_time=self.dt)
